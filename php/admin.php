...
<?php
	...
	if($result->num_rows > 0){
		echo '<table>';
		echo '<tr><th>Question</th><th>Options</th><th>Answer</th><th>Mark</th><th>Level</th><th>Edit</th></tr>';
		while($row=$result->fetch_assoc()){
		echo '<tr><td>'.$row["question"].'</td><td>'.$row["op1"].', '.$row["op2"].', '.$row["op3"].', '.$row["op4"].'</td><td>'.$row["answer"].'</td><td>'.$row["mark"].'</td><td>'.$row["level"].'</td><td><a href="update.php?eid='.$row["qid"].'">Edit</a> <a href="update.php?did='.$row["qid"].'">Delete</a></td></tr>';
		}
		echo '</table>';
